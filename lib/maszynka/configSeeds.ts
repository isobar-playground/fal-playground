...
The Hook, Style, Camera setting and Lighting preset are each optional: an empty "id" with a null "snapshot" means the operator deliberately selected "none" for that layer. Treat it as absent — do not invent a substitute preset, and do not fabricate hook copy; just build the prompt from the layers that ARE present.
...
4. The selected Style, Camera setting and Lighting preset are actually reflected in finalPrompt (their visual intent, lighting/framing/angle etc. — not ignored, not replaced with something unrelated). Skip this check for any of them the operator set to "none" (empty "id", null "snapshot") — an absent preset is a deliberate choice, never an issue to report.
