...
// Global rules, Priority logic, and RUN presets merged locally — no synthetic
// safety/analysis/builder/reviewer statuses are written. Extend this map, not around
// it, when later slices add stages.
//
...
export const ALLOWED_NEXT: Partial<Record<RunStatus, RunStatus[]>> = {
  run_started: [
    ...
    "content_safety_revise_required",
    "content_safety_blocked",
    // OpenRouter bypass ("— none —"): Global rules / Priority logic / RUN presets merged locally → FAL mapper, no LLM stages.
    "fal_request_mapping_completed",
    "fal_request_mapping_failed",
  ],
